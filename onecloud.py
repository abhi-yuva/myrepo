...
class Service:
    ...
    def toCsv(self):
        return f"{self.name}, {self.image}, {self.status}, {self.rollout_status}, {self.deploy.toCsv()}"
...
def get_deploy(namespace: string, name: string, stable_rs: string, kubeconfig_path: string) -> Deploy:
    deploy = Deploy()
    if name is None:
        return deploy
...
def read_file(file_path: Path, lock: Lock, charts_version: string, primary_kubeconfig_path: string, failover_kubeconfig_path='', write_to_console=False):
